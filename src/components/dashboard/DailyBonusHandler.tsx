...
interface DailyBonusHandlerProps {
  refreshUserData?: () => void; // Add prop to receive refresh function
}
...
function DailyBonusHandler({ refreshUserData }: DailyBonusHandlerProps) {
    ...
    if (force || todayJST !== lastProcessedDate) {
      ...
      try {
        ...
        if (data && data.success && data.points_earned > 0) {
          toast.success(
            `今日のログインボーナス！ ${data.points_earned}ポイント獲得！ 🔥連続 ${data.streak}日目！`,
            { duration: 6000 } // Set duration to 6 seconds
          );
          // Call refreshUserData if provided to update data in parent
          if (refreshUserData) {
            refreshUserData();
          }
  ...
  }, [user, isLoaded, hasProcessed, lastProcessedDate, refreshUserData]);
